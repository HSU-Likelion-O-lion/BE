...
class EssayPdfGeneratorTest {
    ...
    @Test
    void generatesPdfWithoutWatermarkWhenDisabled() {
        EssayDetailResponse detail = new EssayDetailResponse(
                7L, "흔들려도 걷는 마음", EssayStatus.COMPLETED, null, null,
                List.of(new EssayDetailResponse.Chapter(1, "1장", List.of("사유 1"))));

        byte[] pdf = generator.generate(detail, false);
    ...
    @Test
    void generatesPdfWithWatermarkWhenEnabled() {
        EssayDetailResponse detail = new EssayDetailResponse(
                7L, "흔들려도 걷는 마음", EssayStatus.COMPLETED, null, null,
                List.of(new EssayDetailResponse.Chapter(1, "1장", List.of("사유 1"))));

        byte[] pdf = generator.generate(detail, true);
