...
BOOK_API_ALADIN_TTB_KEY=발급받은_알라딘_OpenAPI_인증키
```
...
1. 카카오 앱을 생성하고 REST API 키를 확인합니다.
2. 알라딘 OpenAPI 인증키를 발급받고 사용할 서비스 URL을 등록합니다.
3. 로컬 실행 환경에 위 환경 변수를 등록합니다.
4. `GET /api/books/search?q=아몬드`로 외부 검색 결과를 확인합니다.
5. 카카오 키를 제거해 알라딘 fallback이 동작하는지 확인합니다.
6. 두 키를 제거한 뒤 로컬 DB fallback이 동작하는지 확인합니다.
7. 실제 키는 PR이나 이슈에 남기지 않고, 팀 배포 환경에도 동일한 이름으로 등록합니다.

국립중앙도서관 Open API는 추후 ISBN 기반 상세 조회나 도서 메타데이터 보강이 필요할 때 동일한 `BookSearchProvider` 확장 지점에 추가할 수 있습니다.
